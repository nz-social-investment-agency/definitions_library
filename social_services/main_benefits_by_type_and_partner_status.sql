...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_abt_main_benefit_final]
...
DROP VIEW IF EXISTS [DL-MAA20XX-YY].[tmp_main_benefit_spell_input];
GO
DROP VIEW IF EXISTS [DL-MAA20XX-YY].[tmp_main_benefit_partner_input];
GO

/* view for main recipient spells */
CREATE VIEW [DL-MAA20XX-YY].[tmp_main_benefit_spell_input] AS
SELECT [snz_uid]
...
FROM [IDI_Clean_YYYYMM].[msd_clean].[msd_spell]
...
CREATE VIEW [DL-MAA20XX-YY].[tmp_main_benefit_partner_input] AS
SELECT [snz_uid]
      ,[partner_snz_uid]
      ,[msd_ptnr_ptnr_from_date] AS [start_date]
      ,COALESCE([msd_ptnr_ptnr_to_date], '9999-01-01') AS [end_date]
FROM [IDI_Clean_YYYYMM].[msd_clean].[msd_partner]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_condensed];
GO
...
spell_starts AS (
	SELECT [snz_uid]
	    ...
	    ,[start_date]
	    ,[end_date]
	FROM [IDI_UserCode].[DL-MAA20XX-YY].[tmp_main_benefit_spell_input] s1
	WHERE NOT EXISTS (
		SELECT 1
		FROM [IDI_UserCode].[DL-MAA20XX-YY].[tmp_main_benefit_spell_input] s2
...
spell_ends AS (
	SELECT [snz_uid]
	    ...
	    ,[start_date]
	    ,[end_date]
	FROM [IDI_UserCode].[DL-MAA20XX-YY].[tmp_main_benefit_spell_input] t1
	WHERE NOT EXISTS (
		SELECT 1 
		FROM [IDI_UserCode].[DL-MAA20XX-YY].[tmp_main_benefit_spell_input]  t2
...
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_condensed]
FROM spell_starts s
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_condensed] ([snz_uid])
...
/* drop table before re-creating */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_condensed];
GO
...
spell_starts AS (
	SELECT [snz_uid]
	      ...
	      ,[start_date]
	      ,[end_date]
	FROM [IDI_UserCode].[DL-MAA20XX-YY].[tmp_main_benefit_partner_input] s1
	WHERE NOT EXISTS (
		SELECT * 
		FROM [IDI_UserCode].[DL-MAA20XX-YY].[tmp_main_benefit_partner_input] s2
		WHERE s1.snz_uid = s2.snz_uid
...
spell_ends AS (
	SELECT [snz_uid]
	      ...
	      ,[start_date]
	      ,[end_date]
	FROM [IDI_UserCode].[DL-MAA20XX-YY].[tmp_main_benefit_partner_input] t1
	WHERE NOT EXISTS (
		SELECT * 
		FROM [IDI_UserCode].[DL-MAA20XX-YY].[tmp_main_benefit_partner_input]  t2
		WHERE t2.snz_uid = t1.snz_uid
...
SELECT s.snz_uid
	...
	,s.[start_date]
	,MIN(e.[end_date]) as [end_date]
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_condensed]
FROM spell_starts s
INNER JOIN spell_ends e
ON s.snz_uid = e.snz_uid
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_condensed] ([snz_uid])
...
/* drop table before re-creating */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_invert];
GO
...
SELECT [snz_uid]
	  ...
	  ,[start_date]
	  ,[end_date]
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_invert]
FROM (
	...
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_condensed]
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_invert] ([snz_uid])
...
/* drop table before re-creating */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_invert];
GO
...
SELECT [snz_uid]
	  ...
	  ,[start_date]
	  ,[end_date]
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_invert]
FROM (
	/* all forward looking spells */
	SELECT [snz_uid]
		  ,DATEADD(DAY, 1, [end_date]) AS [start_date]
		  ,LEAD(DATEADD(DAY, -1, [start_date]), 1, '9999-01-01') OVER (
				PARTITION BY [snz_uid]
				ORDER BY [start_date] ) AS [end_date]
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_condensed]
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_invert] ([snz_uid])
...
/* drop table before re-creating */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_abt_main_benefit_final];
GO
...
SELECT k.*
	...
	,code.level3
	,code.level4
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_abt_main_benefit_final]
FROM (
...
/* recipient where role = single as no partner during period */
	...
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_condensed] ys -- yes, spell
	INNER JOIN [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_invert] np -- no, partner
...
/* recipient where role = single as never had partner */
	...
	WHERE NOT EXISTS (
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_invert] np -- no, partner
...
/* recipient where role = primary */
	SELECT ys.[snz_uid]
		...
		,ys.[msd_spel_add_servf_code]
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_condensed] ys -- yes, spell
	INNER JOIN [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_condensed] yp -- yes, partner
...
/* receipt as role = partner */
	...
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_partner_condensed] yp -- yes, partner
	INNER JOIN [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_main_benefit_spell_condensed] ps -- (benefit receiving) partner spell
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_abt_main_benefit_final] ([snz_uid])

ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_abt_main_benefit_final] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
