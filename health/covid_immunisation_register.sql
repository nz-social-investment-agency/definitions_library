...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_clean_moh_CIR_vaccination]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_cir_pivot]
GO
...
SELECT	snz_moh_uid 
	...
	,[sequence_group]
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_cir_pivot]
FROM [IDI_Adhoc].[clean_read_MOH_CIR].[moh_CIR_vaccination_activity20211123]
GROUP BY snz_moh_uid, [sequence_group]
GO

CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_cir_pivot] (snz_moh_uid)
GO

-- Merge on snz_uids
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_cir_join_uids]
GO
...
SELECT COALESCE(b.snz_uid,c.snz_uid) AS snz_uid
	...
	,Dose_3_DHB
	,[sequence_group]
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_cir_join_uids]
FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_cir_pivot] AS a
LEFT JOIN [IDI_Clean_YYYYMM].[moh_clean].[pop_cohort_demographics] AS b
ON a.snz_moh_uid = b.snz_moh_uid
LEFT JOIN [IDI_Clean_YYYYMM].[security].[concordance] AS c
ON a.snz_moh_uid = c.snz_moh_uid

-- Present as final
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_clean_moh_CIR_vaccination];
GO
...
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_clean_moh_CIR_vaccination]
FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_cir_join_uids]

CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_clean_moh_CIR_vaccination] (snz_uid);
GO
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_clean_moh_CIR_vaccination] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
GO
