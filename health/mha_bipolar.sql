...
Intended purpose:
All instances that point to a person having Bipolar
...
- [IDI_Metadata].[clean_read_CLASSIFICATIONS].[moh_labs_lab_test]
...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_bipolar]
...
2) Incidence and prevalence
	...
	For example, if we consider any bipolar indication in the last 10 years, then we observe
	a more likely relationship between bipolar and deprivation. This suggests that people in high
...
3) Some treatments are used to treat this condition and other conditions. This is most common
	...
	- Where a person has other evidence of bipolar, add the treatments for multiple conditions
		to the table.
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_solo]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_solo] (
	snz_uid	INT,
	event_date DATE,
)

/* Diagnosis or treatment used for bipolar and other conditions */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_multi]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_multi] (
	snz_uid	INT,
	event_date DATE,
)
...
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_solo] (snz_uid, event_date)
...
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_multi] (snz_uid, event_date)
SELECT a.[snz_uid]
		,[moh_pha_dispensed_date] AS event_date
FROM [IDI_Clean_YYYYMM].[moh_clean].[pharmaceutical] AS a
INNER JOIN [IDI_Metadata].[clean_read_CLASSIFICATIONS].[moh_dim_form_pack_subsidy_code] AS b
...
OR EXISTS (
	SELECT 1
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[ref_diagnosis] AS r
	WHERE SUBSTRING(CAST(formulation_id AS VARCHAR), 1, LEN(r.code)) = r.code
	AND r.diagnosis = 'bipolar'
	AND r.code_type = 'pharm_formulation'
	AND r.aux = 'multiple'
)
GO
...
WHERE [msd_incp_incrsn_code] = '162'
OR [msd_incp_incrsn95_1_code] = '162'
OR [msd_incp_incrsn95_2_code] = '162'
OR [msd_incp_incrsn95_3_code] = '162'
OR [msd_incp_incrsn95_4_code] = '162'
OR [msd_incp_incapacity_code] = '162'
GO
...
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_solo] (snz_uid, event_date)
SELECT [snz_uid]
	,[moh_lab_visit_date] AS event_date
FROM [IDI_Clean_YYYYMM].[moh_clean].[lab_claims]
WHERE EXISTS (
	SELECT 1
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[ref_diagnosis] AS r
	WHERE SUBSTRING([moh_lab_test_code], 1, LEN(r.code)) = r.code
	AND r.diagnosis = 'bipolar'
	AND r.code_type = 'lab_test'
)
GO
...
/* Add indexes */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_solo] (snz_uid);
GO
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_multi] (snz_uid);
GO

DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_bipolar]
GO

WITH multi_to_add AS (
	SELECT snz_uid, event_date
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_multi] AS m
	WHERE EXISTS (
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_solo] AS s
		WHERE m.snz_uid = s.snz_uid
	)
)
SELECT DISTINCT snz_uid, event_date
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_bipolar]
FROM (
	SELECT snz_uid, event_date
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_bipolar_solo]
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_bipolar] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_bipolar] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
