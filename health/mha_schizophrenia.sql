...
Description:
Indication of Schizophrenia spectrum.
...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_schizophrenia]
...
CREATE TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_solo] (
	snz_uid	INT,
	event_date DATE,
)
...
CREATE TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_multi] (
	snz_uid	INT,
	event_date DATE,
)
...
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_solo] (snz_uid, event_date)
...
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_multi] (snz_uid, event_date)
SELECT a.[snz_uid]
		,[moh_pha_dispensed_date] AS event_date
FROM [IDI_Clean_YYYYMM].[moh_clean].[pharmaceutical] AS a
INNER JOIN [IDI_Metadata].[clean_read_CLASSIFICATIONS].[moh_dim_form_pack_subsidy_code] AS b
...
/* Add indexes */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_solo] (snz_uid);
GO
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_multi] (snz_uid);
GO

DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_schizophrenia]
GO

WITH multi_to_add AS (
	SELECT snz_uid, event_date
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_multi] AS m
	WHERE EXISTS (
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_solo] AS s
		WHERE m.snz_uid = s.snz_uid
	)
)
SELECT DISTINCT snz_uid, event_date
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_schizophrenia]
FROM (
	SELECT snz_uid, event_date
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_schizophrenia_solo]
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_schizophrenia] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[defn_mha_schizophrenia] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
