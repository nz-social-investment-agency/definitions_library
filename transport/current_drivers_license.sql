...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[defn_end_of_month_location]
...
Issues:
1) Created when driver license data in the IDI was supplied by NZTA without complete history.
	Hence necessary to look back at multiple refreshes to create history.
	NZTA supply may have changed - check first before reusing definition.

History (reverse order):
2024-07-01 SA merge into library
2020-05-20 SA v1
**************************************************************************************************/

/******************** Create full list ********************/
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_drivers_license_w_history_staging];
GO

SELECT *
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_drivers_license_w_history_staging]
FROM (
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_drivers_license_w_history_staging] ([snz_nzta_uid]);
GO

/******************** Create tranlation ********************/
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_drivers_license_w_history_uid];
GO

SELECT DISTINCT snz_uid, [snz_nzta_uid]
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_drivers_license_w_history_uid]
FROM [IDI_Clean_20200120].[nzta_clean].[drivers_licence_register]

/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_drivers_license_w_history_uid] ([snz_nzta_uid]);
GO

/******************** Create final ********************/
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[defn_drivers_license_w_history];
GO
...
SELECT new.snz_uid
      ...
      ,old.[nzta_dlr_licence_from_date]
	  ,old.next_date
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[defn_drivers_license_w_history]
FROM (
	...
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_drivers_license_w_history_staging]

) old
INNER JOIN [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_drivers_license_w_history_uid] new
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[defn_drivers_license_w_history] ([snz_uid]);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[defn_drivers_license_w_history] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
