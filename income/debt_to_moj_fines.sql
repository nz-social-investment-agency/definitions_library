...
2. Calculating number of debtors and total value of debts.
...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_transactions_ready]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_debt_full_summary_trimmed];
GO

SELECT *
      ,EOMONTH(DATEFROMPARTS(year_nbr, month_of_year_nbr, 1)) AS month_date
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_debt_full_summary_trimmed]
FROM [IDI_Adhoc].[clean_read_DEBT].[moj_debt_full_summary_trimmed]
WHERE year_nbr > 2011
GO

/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_debt_full_summary_trimmed] ([moj_ppn]);
GO
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_tmp_moj_debt_prep];
GO

SELECT *
	  ,SUM(balance_before_2012 + delta) OVER (PARTITION BY moj_ppn ORDER BY month_date) AS balance_correct
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_tmp_moj_debt_prep]
FROM(
	SELECT *
	...
	FROM(
		...
		FROM [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_debt_full_summary_trimmed]
	)a
)b

/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_tmp_moj_debt_prep] ([moj_ppn]);
GO

/**************************************************************************************************
fill in records where balance is non-zero but transactions are zero
**************************************************************************************************/
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_non_transactions];
GO
...
FROM [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_tmp_moj_debt_prep]
),
joined AS (
	SELECT *
	FROM debt_source
...
SELECT * 
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_non_transactions]
FROM (
	/* original records */
	SELECT moj_ppn 
	      ,month_date
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_non_transactions] ([moj_ppn]);
GO
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_debt_id];
GO
...
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_debt_id]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_transactions_ready];
GO

SELECT b.snz_uid
       ,a.*
INTO  [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_transactions_ready]
FROM [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_non_transactions] a
LEFT JOIN [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_debt_id] b
ON a.moj_ppn = b.moj_ppn

/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[d2gP2_moj_transactions_ready] (snz_uid);
GO
