...
Description:
Summary description of a person's neighbourhood including: region, deprivation, urban/rural, and whether a person lives in a household with dependent children.

Intended purpose:
Identifying the region, urban/rural-ness, and other characteristics of where a person lives at a specific point in time.
...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[defn_address_descriptors_yearly]
...
2022-05-17 VW    Point to MSD seniors project (DL-MAA20XX-YY), update to latest refresh, add NZDep2018 for use with different years (will need to edit code below)
2022-02-18 VW    Point to HTR project, DL-MAA20XX-YY
...
/* Remove table */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[defn_address_descriptors_yearly];  -- rename 
GO
...
FROM [IDI_Clean_YYYYMM].[data].[address_notification]
)
...
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[defn_address_descriptors_yearly]
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[defn_address_descriptors_yearly] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[defn_address_descriptors_yearly] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
