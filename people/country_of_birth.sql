...
Intended purpose:
1. Supplement ethnicity and identity information by including Country of Birth.
2. Where country of birth (COB) is not available we instead use country of citizenship (COC).
...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_country_of_birth]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_COB_list]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_COB_list] (
	snz_uid INT,
	c_type VARCHAR(5),
	code VARCHAR(5),
	source_rank INT,
...
'V14.1' as code_sys
********************************************************/
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_COB_list] (snz_uid, c_type, code, source_rank)
SELECT [snz_uid]
	,'COB' AS c_type
...
SELECT [snz_uid]
	...
	,4 AS source_rank
FROM [IDI_Clean_YYYYMM].[cus_clean].[journey]
...
FROM (
	...
	FROM [IDI_Clean_YYYYMM].[nzta_clean].[dlr_historic]
...
SELECT DISTINCT [snz_uid]
	...
	,CASE [dol_mid_birth_country_code]
		...
		ELSE NULL END AS code
	,6 AS source_rank
FROM [IDI_Clean_YYYYMM].[dol_clean].[movement_identities]
GO
...
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_moe_enrollment]
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_moe_enrollment] (snz_uid)
GO

WITH date_ranked AS (
	SELECT *
		,RANK() OVER (PARTITION BY snz_uid ORDER BY moe_enr_prog_start_date) AS date_rank
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_moe_enrollment]
)
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_COB_list] (snz_uid, c_type, code, source_rank)
SELECT snz_uid
	,'COC' AS c_type
...
'XXXX' as raw_code_sys
********************************************************/
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_moe_school_roll]
GO

SELECT DISTINCT [snz_moe_uid]
	,[CollectionDate]
	,[CountryOfCitizenship]
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_moe_school_roll]
FROM (
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_moe_school_roll] ([snz_moe_uid])
GO

WITH date_ranked AS (
	SELECT *
		,RANK() OVER (PARTITION BY [snz_moe_uid] ORDER BY [CollectionDate]) AS date_rank
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_moe_school_roll]
)
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_COB_list] (snz_uid, c_type, code, source_rank)
SELECT snz_uid
	,'COC' AS c_type
...
INNER JOIN [IDI_Clean_YYYYMM].[moe_clean].[nsi] AS b
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_COB_list] (snz_uid)
GO

DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_country_of_birth]
GO

WITH source_ranked AS (
	SELECT *
		,RANK() OVER (PARTITION BY [snz_uid] ORDER BY source_rank) AS ranked
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_COB_list]
)
SELECT snz_uid
	, c_type
	, CAST(code AS INT) AS code
	, source_rank
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_country_of_birth]
FROM source_ranked
WHERE ranked = 1
...
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_country_of_birth] (snz_uid);
GO
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_country_of_birth] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
GO
