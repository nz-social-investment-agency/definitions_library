...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_ethnicity_level_2]
- [IDI_UserCode].[DL-MAA20XX-YY].[vacc_ethnicity_level_1_and_2]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_ethnicity_list]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_ethnicity_list] (
	snz_uid INT,
...
INSERT INTO [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_ethnicity_list] (snz_uid, ethnic_code, source_rank, record_date)
SELECT [snz_uid]
...
FROM [IDI_Clean_YYYYMM].[moh_clean].[pop_cohort_demographics]
WHERE snz_uid IS NOT NULL
GO
...
SELECT b.snz_uid
	...
	,CAST('YYYY-MM-DD' AS DATE) AS record_date
...
FROM [IDI_Clean_YYYYMM].[moe_clean].[student_per]
...
SELECT [snz_uid]
	...
	,CAST('YYYY-MM-DD' AS DATE) AS record_date
FROM [IDI_Clean_YYYYMM].[acc_clean].[clients]
WHERE snz_uid IS NOT NULL
GO
...
SELECT [snz_uid]
	...
	,CAST('YYYY-MM-DD' AS DATE) AS record_date
FROM [IDI_Clean_YYYYMM].[msd_clean].[msd_swn]
WHERE snz_uid IS NOT NULL
GO
...
FROM [IDI_Clean_YYYYMM.[sla_clean].[msd_borrowing]
WHERE snz_uid IS NOT NULL
GO
...
FROM [IDI_Clean_YYYYMM].[sofie_clean].[person_waves] AS a
INNER JOIN [IDI_Clean_YYYYMM].[sofie_clean].[hq_id] AS b
...
FROM [IDI_Clean_YYYYMM].[hlfs_clean].[data] AS a
INNER JOIN [IDI_Clean_YYYYMM].[hlfs_clean].[nzis] AS b
ON a.snz_uid = b.snz_uid
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_ethnicity_list] (snz_uid)
GO

DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_ethnicity_level_2]
GO

WITH source_ranked AS (
	SELECT *
		,RANK() OVER (PARTITION BY [snz_uid] ORDER BY source_rank, record_date) AS ranked
	FROM [IDI_Sandpit].[DL-MAA20XX-YY].[tmp_ethnicity_list]
)
SELECT snz_uid
	,ethnic_code
	,source_rank
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_ethnicity_level_2]
FROM source_ranked
WHERE ranked = 1
AND snz_uid IS NOT NULL
GO

CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_ethnicity_level_2] (snz_uid);
GO
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_ethnicity_level_2] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
GO
...
DROP VIEW IF EXISTS [DL-MAA20XX-YY].[vacc_ethnicity_level_1_and_2]
GO

CREATE VIEW [DL-MAA20XX-YY].[vacc_ethnicity_level_1_and_2] AS
SELECT snz_uid
	,ethnic_code
	,source_rank
	/* level 1 ethnicities */
...
FROM [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_ethnicity_level_2]
GO
