...
Intended purpose:
Identify the periods of enrolment in education.
...
Outputs:
- [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_moe_tidy_dates]
- [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_education_period];
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_moe_tidy_dates];
GO
...
initial_setup AS (
	...
	FROM [IDI_Clean_YYYYMM].[moe_clean].[student_enrol] enr
	LEFT JOIN [IDI_Clean_YYYYMM].[data].[personal_detail] per
...
SELECT DISTINCT snz_uid
	...
	,moe_esi_extrtn_date
	,moe_esi_provider_code /* school number */
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_moe_tidy_dates]
...
--put index
CREATE INDEX individx ON [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_moe_tidy_dates] (snz_uid);
GO
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_moe_tidy_dates] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
GO
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_education_period];
GO
...
*/
SELECT *
INTO [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_education_period]
FROM (
...
FROM [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_moe_tidy_dates]
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_education_period] (snz_uid)
GO
ALTER TABLE [IDI_Sandpit].[DL-MAA20XX-YY].[vacc_education_period] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
GO
